...
- `lesson2.html` — 2차시. lesson1의 셸(헤더·TTS·볼륨·진행 점·칭찬·축하)을 복사해 만들었고, 스텝 타입은 `info`/`findkey`(위치 찾기)/`typeword`(따라 쓰기) 세 개.
  - `typeword`: 목표는 완성형(`도리`)으로 보여주고 입력칸은 실시간 조합(`composeHangul`). 틀린 자모도 **입력을 허용**해 빨갛게 남기고, 지우기로 바로잡아야 완성된다(`S.typed` 배열이 상태). 파트(모듈) 2개: '키보드 위치 찾기'·'글자 따라 쓰기'. 내부 홈(카드 선택) 화면에 index.html로 나가는 '다른 수업 고르기' 링크.
